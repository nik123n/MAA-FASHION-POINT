    ...
    <footer className="relative overflow-hidden bg-[#19204a] text-gray-200 pt-16 pb-24 md:pb-8 border-t-[6px] border-leaf-500 pb-safe">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          ...
          {/* Quick Links Section */}
          <div>
            <h4 className="font-display text-white text-lg mb-6 border-b border-white/10 pb-2 inline-block">{t('footer.quick_links')}</h4>
            <ul className="grid grid-cols-2 gap-y-3 gap-x-6">
              {[
                { label: t('footer.links.new'), to: '/products?isNewArrival=true' },
                { label: t('footer.links.trending'), to: '/products?isTrending=true' },
                { label: t('footer.links.all'), to: '/products' },
                { label: t('profile.title'), to: '/profile' },
                { label: t('profile.orders'), to: '/orders' },
                { label: t('profile.wishlist'), to: '/wishlist' },
                { label: t('footer.links.cart'), to: '/cart' },
              ].map((item) => (
                <li key={item.label}>
          ...
          {/* Contact Section */}
          <div>
            <h4 className="font-display text-white text-lg mb-6 border-b border-white/10 pb-2 inline-block">{t('footer.contact')}</h4>
